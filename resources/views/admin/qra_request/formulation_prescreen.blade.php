...
<form method="POST" action="{{ route('qra_request.edit_formulation_prescreen', $qra_request_type_id) }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="formulation_prescreen">
        ...
        <p style="color: #222; text-align: center; font-weight: bold;">FORMULATION PRESCREEN</p>
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="version">
                ...
                <option value="{{ $val }}" {{ $data[0]->version == $val ? 'selected' : '' }} >
                    {{ $val }}
                </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="form-group">
            ...
            <input type="text" name="material_number" class="form-control" value="{{ $data[0]->material_number }}">
        </div>

        <div class="form-group">
            <label>Vendor Code:</label>
            <input type="text" name="vendor_code" class="form-control" id="{{ $qra_request_type_id }}_vendor_auto" value="{{ $data[0]->vendor_code }}">
        </div>

        <div class="form-group">
            <label>Vendor Name:</label>
            <input type="text" name="vendor_name" class="form-control" id="{{ $qra_request_type_id }}_vendor_name" value="{{ $data[0]->vendor_name }}" readonly>
        </div>
    ...
    <div class="form-group">
        ...
        <?php if(auth()->user()->role == 'Admin' || auth()->user()->team == $data[0]->team) { ?>
            <input type="button"
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $("#{{ $qra_request_type_id }}_vendor_auto").autocomplete({
            ...
            select:function(e,selectedData) {
                ...
                if(selectedData && selectedData.item && selectedData.item.data){
                    ...
                    $('#{{ $qra_request_type_id }}_vendor_name').val(data.name);
                }
            }
        });
    });
