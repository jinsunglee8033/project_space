...
<form method="POST" action="{{ route('project.add_concept_development') }}" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input required type="text" name="{{ $task_type }}_delivery_date" id="new_delivery_date" required autocomplete="off"
               class="form-control @error($task_type.'_delivery_date') is-invalid @enderror @if (!$errors->has($task_type.'_delivery_date') && old($task_type.'_delivery_date')) is-valid @endif"
               value="{{ old($task_type . '_delivery_date', null) }}">
    </div>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        //     }
        // }
        $('input[name="<?php echo $task_type; ?>_delivery_date"]').daterangepicker({
        ...
        $('input[name="<?php echo $task_type; ?>_delivery_date"]').val('');
    });
</script>
