                ...
                <div class="col-lg-7">
                    ...
                    @if(!empty($request_type_list))
                        ...
                        <?php foreach ($request_type_list as $task): ?>
                            ...
                            <div class="card assets_existing">
                                ...
                                ?>
                                <div class="clearfix" id="{{$task->ra_request_type_id}}" style="border-left: 10px solid {{ $left_border_color }}; border-radius: 20px;">

                                    <div class="asset--grid-row">

                                        <div class="project-info">
                                            <ul class="project-info-list">
                                                <li><strong>{{ strtoupper(str_replace('_', ' ', $task->request_type)) }} #{{ $task->ra_request_type_id }}</strong> </li>
                                                <?php if ($left_border_color == 'white') $left_border_color = '#6c757d'; ?>
                                                <li><strong>Status: </strong> <b style="color: {{ $left_border_color }};">
                                                        <?php if($task->status == 'action_review'){?>
                                                        REVISION
                                                        <?php }else{?>
                                                        {{ mb_strtoupper(str_replace('_', ' ', $task->status)) }}
                                                        <?php }?>
                                                    </b>
                                                </li>
                                                <li><strong>Due Date:
                                            ...
                                            <div class="project-action-icons">
                                                <ul class="project-icons">
                                                    <li>
                                                        ...
                                                        class="close"
                                                        data-id=""
                                                        data-ra-request-type-id="{{ $task->ra_request_type_id }}"
                                                        data-ra-request-type="{{ $task->request_type }}"
                                                        onclick="delete_ra_request_type($(this));">
                                    ...
                                    <div style="text-align: center;">
                                        <i id="arrow-{{$task->ra_request_type_id}}" class="dropdown fa-lg fa fa-angle-down" onclick="click_arrow(this, {{$task->ra_request_type_id}})"></i>
                                    </div>

                                    <div id="asset-id-{{$task->ra_request_type_id}}" class="box-body form_creator" data-asset-id="{{ $task->ra_request_type_id }}" style="display: none">
                                        <section>
                                            <div class="inner_box">
                                                <?php $data = [$task, $task->files]; ?>
                                                @include('admin.ra_request.'.$task->request_type, $data)
                                            </div>
                                        </section>
                                    </div>
                    ...
                    @if(!empty($project))
                        ...
                        <div class="card box asset box-primary">
                            <div class="card-header" id="ra_bg" style="height: 500px;">
                                        ...
                                        <div class="inner_box">
                                            <div class="form-group">
                                                ...
                                                <span class="float-right">
                                                    <a href="{{ url('admin/ra_request/'. $project->id .'/edit') }}">
                                                ...
                                                <select name="add_task_type" id="add_request_type" class="form-control" onchange="select_request_type()">
                                                    <option value="">Select</option>
                                                    <option value="formula_review">Formula Review</option>
                                                    <option value="label_review">Label Review</option>
                                                    <option value="us_launch">US Launch (WERCS, Smarter X, California, MoCRA)</option>
                                                    <option value="canada_launch">Canada Launch (CNF)</option>
                                                    <option value="eu_launch">EU Launch (CPNP)</option>
                                                    <option value="uk_launch">UK Launch (SCPN)</option>
{{--                                                    <option value="latam_launch">LATAM Launch (IIO)</option>--}}
                                                </select>

                                            </div>

                                            <div id="new_formula_review" style="display: none;">
                                                @include('admin.ra_request.new.formula_review')
                                            </div>
                                            <div id="new_label_review" style="display: none;">
                                                @include('admin.ra_request.new.label_review')
                                            </div>
                                            <div id="new_us_launch" style="display: none;">
                                                @include('admin.ra_request.new.us_launch')
                                            </div>
                                            <div id="new_canada_launch" style="display: none;">
                                                @include('admin.ra_request.new.canada_launch')
                                            </div>
                                            <div id="new_eu_launch" style="display: none;">
                                                @include('admin.ra_request.new.eu_launch')
                                            </div>
                                            <div id="new_uk_launch" style="display: none;">
                                                @include('admin.ra_request.new.uk_launch')
                                            </div>
                                            <div id="new_latam_launch" style="display: none;">
                                                @include('admin.ra_request.new.latam_launch')
                                            </div>
                            ...
                            <div class="col">
                                ...
                                <section id="add_note" class="notes" style="display: none;">
                                    <div class="write note">
                                        <form method="POST" action="{{ route('ra_request.add_note') }}" enctype="multipart/form-data">
                                            @csrf
    ...
    <script type="text/javascript">
        ...
        function select_request_type(){
            ...
            if(request_type == 'formula_review'){
                $("#new_formula_review").fadeIn(600);
                $("#new_label_review").fadeOut();
                $("#new_us_launch").fadeOut();
                $("#new_canada_launch").fadeOut();
                $("#new_eu_launch").fadeOut();
                $("#new_uk_launch").fadeOut();
                $("#new_latam_launch").fadeOut();
            }else if(request_type == 'label_review'){
                $("#new_formula_review").fadeOut();
                $("#new_label_review").fadeIn(600);
                $("#new_us_launch").fadeOut();
                $("#new_canada_launch").fadeOut();
                $("#new_eu_launch").fadeOut();
                $("#new_uk_launch").fadeOut();
                $("#new_latam_launch").fadeOut();
            }else if(request_type == 'us_launch'){
                $("#new_formula_review").fadeOut();
                $("#new_label_review").fadeOut();
                $("#new_us_launch").fadeIn(600);
                $("#new_canada_launch").fadeOut();
                $("#new_eu_launch").fadeOut();
                $("#new_uk_launch").fadeOut();
                $("#new_latam_launch").fadeOut();
            }else if(request_type == 'canada_launch'){
                $("#new_formula_review").fadeOut();
                $("#new_label_review").fadeOut();
                $("#new_us_launch").fadeOut();
                $("#new_canada_launch").fadeIn(600);
                $("#new_eu_launch").fadeOut();
                $("#new_uk_launch").fadeOut();
                $("#new_latam_launch").fadeOut();
            }else if(request_type == 'eu_launch'){
                $("#new_formula_review").fadeOut();
                $("#new_label_review").fadeOut();
                $("#new_us_launch").fadeOut();
                $("#new_canada_launch").fadeOut();
                $("#new_eu_launch").fadeIn(600);
                $("#new_uk_launch").fadeOut();
                $("#new_latam_launch").fadeOut();
            }else if(request_type == 'uk_launch'){
                $("#new_formula_review").fadeOut();
                $("#new_label_review").fadeOut();
                $("#new_us_launch").fadeOut();
                $("#new_canada_launch").fadeOut();
                $("#new_eu_launch").fadeOut();
                $("#new_uk_launch").fadeIn(600);
                $("#new_latam_launch").fadeOut();
            }else if(request_type == 'latam_launch'){
                $("#new_formula_review").fadeOut();
                $("#new_label_review").fadeOut();
                $("#new_us_launch").fadeOut();
                $("#new_canada_launch").fadeOut();
        ...
        function delete_ra_request_type(el) {
            if (confirm("Are you sure to Delete?") == true) {
                let id = $(el).attr('data-ra-request-type-id');
                let type = $(el).attr('data-ra-request-type');
                $.ajax({
                    url: "<?php echo url('/admin/ra_request/requestTypeRemove'); ?>"+"/"+id+"/"+type,
                    type: "GET",
                    datatype: "json",
        ...
        function remove_file(el) {
            if (confirm("Are you sure to Delete File?") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/ra_request/fileRemove'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response == 'success'){
        ...
        function action_start(el){
            ...
            if (confirm("Your request status will change to 'In Progress'") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/ra_request/actionInProgress'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function ra_revision(el){

            if (confirm("Are you sure to RA Revision?") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/ra_request/raRevision'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function ra_resubmit(el){

            if (confirm("Are you sure to RA Re-Submit?") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/ra_request/raResubmit'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function review_complete(el){

            if (confirm("The request is now complete, and no further changes can be made.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/ra_request/actionComplete'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
