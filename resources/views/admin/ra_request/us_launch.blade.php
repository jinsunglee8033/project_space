...
<form method="POST" action="{{ route('ra_request.edit_us_launch', $ra_request_type_id) }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="registration_wercs">
        ...
        <p style="color: #222; text-align: center; font-weight: bold;">US LAUNCH</p>
            ...
            <div class="form-group">
                <label>Requested Due Date: <b style="color: #b91d19">(The request will take 5 business days to complete.)</b></label>
                <input type="text" name="due_date" class="form-control" readonly value="{{ $data[0]->due_date }}">
            </div>

            <div class="form-group">
                <label>Market: (WERCS or SmarterX registration will be sent to all retailers, unless specifically identified by Product Division)</label>
                <select class="form-control" name="market" required>
                    <option value="">Select</option>
                    <?php foreach($market_list as $val): ?>
                    <option value="{{ $val }}" {{ $data[0]->market == $val ? 'selected' : '' }} >
                        {{ $val }}
                    </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label>Primary Vendor Code: <b style="color: #b91d19">(Auto-Complete)</b></label>
                        <input type="text" name="bulk_vendor_code" id="{{$ra_request_type_id}}_bulk_vendor_code" class="form-control" value="{{ $data[0]->bulk_vendor_code }}" required>
                    </div>
                </div>
{{--                <div class="col-md-6">--}}
{{--                    <div class="form-group">--}}
{{--                        <label>Primary Vendor Name:</label>--}}
{{--                        <input type="text" name="bulk_vendor_name" id="{{$ra_request_type_id}}_bulk_vendor_name" class="form-control" readonly value="{{ $data[0]->bulk_vendor_name }}">--}}
{{--                    </div>--}}
{{--                </div>--}}
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="filling_vendor_code" id="{{$ra_request_type_id}}_filling_vendor_code" class="form-control" value="{{ $data[0]->filling_vendor_code }}">
                    </div>
                </div>
{{--                <div class="col-md-6">--}}
{{--                    <div class="form-group">--}}
{{--                        <label>Filling Vendor Name:</label>--}}
{{--                        <input type="text" name="filling_vendor_name" id="{{$ra_request_type_id}}_filling_vendor_name" class="form-control" readonly value="{{ $data[0]->filling_vendor_name }}">--}}
{{--                    </div>--}}
{{--                </div>--}}
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="packaging_vendor_code" id="{{$ra_request_type_id}}_packaging_vendor_code" class="form-control" value="{{ $data[0]->packaging_vendor_code }}">
                    </div>
                </div>
{{--                <div class="col-md-6">--}}
{{--                    <div class="form-group">--}}
{{--                        <label>Packaging Vendor Name:</label>--}}
{{--                        <input type="text" name="packaging_vendor_name" id="{{$ra_request_type_id}}_packaging_vendor_name" class="form-control" readonly value="{{ $data[0]->packaging_vendor_name }}">--}}
{{--                    </div>--}}
{{--                </div>--}}
            </div>

            <?php if (count($data[1]) > 0): ?>
                <label style="font-weight: 800; color: #34395e; font-size: 12px;">Upload Artwork: </label>
            <br/>
            ...
            <div style="background-color: #e9e9e9; padding: 10px 10px 10px 10px; margin: 0 0 10px 0;">
                ...
                <div class="form-group">
                    <label>Est. Registration Due Date: <b style="color: #b91d19">(Select from the calendar)</b></label>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $("#{{ $ra_request_type_id }}_bulk_vendor_code").autocomplete({
            ...
            select:function(e,selectedData) {
                ...
                if(selectedData && selectedData.item && selectedData.item.data){
                    ...
                    $('#{{ $ra_request_type_id }}_bulk_vendor_name').val(data.name);
                }
            }
        });
        $("#{{ $ra_request_type_id }}_filling_vendor_code").autocomplete({
            source: function (request, cb){
                $.ajax({
                    url: "<?php echo url('/admin/project/autocomplete_vendor'); ?>"+"?code="+request.term,
                    method: 'GET',
            ...
            select:function(e,selectedData) {
                ...
                if(selectedData && selectedData.item && selectedData.item.data){
                    var data = selectedData.item.data;
                    $('#{{ $ra_request_type_id }}_filling_vendor_name').val(data.name);
                }
            }
        });
        $("#{{ $ra_request_type_id }}_packaging_vendor_code").autocomplete({
            source: function (request, cb){
                $.ajax({
                    url: "<?php echo url('/admin/project/autocomplete_vendor'); ?>"+"?code="+request.term,
                    method: 'GET',
            ...
            select:function(e,selectedData) {
                ...
                if(selectedData && selectedData.item && selectedData.item.data){
                    var data = selectedData.item.data;
                    $('#{{ $ra_request_type_id }}_packaging_vendor_name').val(data.name);
                }
            }
        });
    });
