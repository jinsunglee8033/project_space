...
@section('content')
    ...
    <section class="section">
        ...
        <div class="section-header">
            <h1>NPD Planner Request</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></div>
                <div class="breadcrumb-item"><a href="{{ url('admin/project') }}">NPD Planner Request</a></div>
                <div class="breadcrumb-item active">NPD Planner Request</div>
            </div>
        </div>

        <div class="section-body">
            <h2 class="section-title">NPD Planner Request</h2>
            <div class="row">
                <div class="col-lg-7">
                    <div class="card">
                    ...
                    @if(!empty($npd_planner_request_list))

                        <?php foreach ($npd_planner_request_list as $task): ?>
{{--                        if Exist!!!   --}}
                        <?php
                        ...
                        ?>
                            ...
                            <div class="card assets_existing">
                                ...
                                <div class="clearfix" id="{{$task->npd_planner_request_type_id}}" style="border-left: 10px solid {{ $left_border_color }}; border-radius: 20px;">

                                    <div class="asset--grid-row">

                                        <div class="project-info">
                                            <ul class="project-info-list">
                                                <li><strong>{{ strtoupper(str_replace('_', ' ', $task->request_type)) }} #{{ $task->npd_planner_request_type_id }}</strong> </li>
                                                <?php if ($left_border_color == 'white') $left_border_color = '#6c757d'; ?>
                                                <li><b style="color: #b91d19;">{{ $task->request_group }} </b></li>
                                                ...
                                                <ul class="project-icons">
                                                    <li>
                                                        <a  href="javascript:void(0);"
                                                            class="close"
                                                            data-id=""
                                                            data-request-type-id="{{ $task->npd_planner_request_type_id }}"
                                                            data-request-type="{{ $task->request_type }}"
                                                            onclick="delete_npd_planner_request_type($(this));">
                                    ...
                                    <div style="text-align: center;">
                                        <i id="arrow-{{$task->npd_planner_request_type_id}}" class="dropdown fa-lg fa fa-angle-down" onclick="click_arrow(this, {{$task->npd_planner_request_type_id}})"></i>
                                    </div>

                                    <div id="asset-id-{{$task->npd_planner_request_type_id}}" class="box-body form_creator" data-asset-id="{{ $task->npd_planner_request_type_id }}" style="display: none">
                                        <section>
                                            <div class="inner_box">
                                                <?php $data = [$task, $task->files]; ?>
                                                @include('admin.npd_planner_request.'.$task->request_type, $data)
                                            </div>
                                        </section>
                                    </div>
                        ...
                        <?php if(Auth::user()->function == 'Product' || Auth::user()->function == 'Admin') {?>
                            ...
                            <div class="clearfix" id="asset_selector" style="display: block">
                                <div class="card box asset box-primary">
                                    <div class="card-header" id="planner_bg" style="height: 500px;">
                                        <div class="row" style="display: flex; justify-content: center; padding: 5px 0 0 0;">
                                            <div id="add_task_btn" style="display: block">
                                                <a class="btn-lg btn-secondary add-row" style="color: #050000; background-color: #cdd3d8; border-radius: 1.25rem;" onclick="click_task_add_btn()">Add <b style="color: #b91d19;">NPD PLANNER REQUEST</b> Type</a>
                                            </div>
                                        </div>
                                                ...
                                                <div class="inner_box">
                                                    <div class="form-group">
                                                        ...
                                                        <span class="float-right">
                                                            <a href="{{ url('admin/npd_planner_request/'. $project->id .'/edit') }}">
                                                        ...
                                                        <select name="add_task_type" id="add_request_type" class="form-control" onchange="select_request_type()">
                                                            <option value="">Select</option>
                                                            <option value="project_planner">Project Planner</option>
                                                            <option value="presale_plan">Presale Plan</option>
                                                    ...
                                                    <div class="form-group">
                                                        <label>
                                                            <a href="https://drive.google.com/drive/folders/1TGHDcsUlBm4yTWCrOB-mxZa6uLT9YnJR" target="_blank" class="badge badge-danger">Download Link</a>
                                                        </label>
                                                    </div>

                                                    <div id="new_project_planner" style="display: none;">
                                                        @include('admin.npd_planner_request.new.project_planner')
                                                    </div>
                                                    <div id="new_presale_plan" style="display: none;">
                                                        @include('admin.npd_planner_request.new.presale_plan')
                                                    </div>
                                                    <div id="new_change_request" style="display: none;">
                                                        @include('admin.npd_planner_request.new.change_request')
                                                    </div>
                                                </div>
                                            </section>
                                        </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class=" text-right">
                                <button class="btn btn-dark" id="add_note_btn" onclick="click_add_note_btn()">Add Note</button>
                            </div>
                        </div>
                            ...
                            <div class="col">
                                ...
                                <section id="add_note" class="notes" style="display: none;">
                                    <div class="write note">
                                        <form method="POST" action="{{ route('npd_planner_request.add_note') }}" enctype="multipart/form-data">
                                            @csrf
    ...
    <script type="text/javascript">
        ...
        function select_request_type(){
            ...
            if(request_type == 'project_planner'){
                $("#new_project_planner").fadeIn(600);
                $("#new_presale_plan").fadeOut();
                $("#new_change_request").fadeOut();
            }else if(request_type == 'presale_plan'){
                $("#new_project_planner").fadeOut();
                $("#new_presale_plan").fadeIn(600);
                $("#new_change_request").fadeOut();
            }else if(request_type == 'change_request'){
                $("#new_project_planner").fadeOut();
                $("#new_presale_plan").fadeOut();
                $("#new_change_request").fadeIn(600);
            }
            $("#planner_bg").height('');
        }
        ...
        function remove_file(el) {
            if (confirm("Are you sure to Delete File?") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_planner_request/fileRemove'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
        ...
        function action_resubmit(el){
            if (confirm("Ensure all changes are saved before resubmitting.") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_planner_request/actionReSubmit'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_start(el){
            ...
            if (confirm("Your request status will change to 'In Progress'") == true) {
                ...
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_planner_request/actionInProgress'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_review(el){
            ...
            if (confirm("The work has been updated for review, and the status will now change to 'Action Review'.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_planner_request/actionReview'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_complete(el){
            ...
            if (confirm("The request is now complete, and no further changes can be made.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_planner_request/actionComplete'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_upload(el){

            if (confirm("Are you sure to Action Upload?") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_planner_request/actionUpload'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function delete_npd_planner_request_type(el) {
            if (confirm("Are you sure you want to delete? Once deleted, this action cannot be undone and the data will no longer be accessible.") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_planner_request/requestTypeRemove'); ?>"+"/"+id+"/"+type,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
