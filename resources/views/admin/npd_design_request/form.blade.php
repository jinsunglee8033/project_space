...
{{--    <script src="https://cloud.tinymce.com/5/tinymce.min.js?apiKey=u0nqw6bdnhchs7yky70br91x6sl6ja1nc4hor8asbmv2ie3a"></script>--}}
    ...
    <style>
        ...
        .asset--grid-row{
            ...
            --multiple-assignees-bg: var(--light-steal-blue);
            display: grid;
            gap: 20px;
        ...
        .multiple-assignees-bg{
            background-color: var(--multiple-assignees-bg);
        }
    ...
    <section class="section">
        ...
        <div class="section-header">
            <h1>NPD Design Request</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></div>
                <div class="breadcrumb-item"><a href="{{ url('admin/project') }}">NPD Design Request</a></div>
                <div class="breadcrumb-item active">NPD Design Request</div>
            </div>
        </div>

        <div class="section-body">
            <h2 class="section-title">NPD Design Request</h2>
            <div class="row">
                <div class="col-lg-7">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="col">
                                ...
                                <div class="form-group">
                                    ...
                                    @endif
                                    <label>Team</label>
                                    <select id="firstSelect" class="form-control" disabled name="team">
                    ...
                    @if(!empty($request_type_list))
                        ...
                        <?php foreach ($request_type_list as $task): ?>
                            ...
                            <div class="card assets_existing">
                                ...
                                }else if($task->status == 'action_review' || $task->status == 'update_required'){
                                ...
                                <?php
                                $design_group = $task->design_group;
                                if($design_group == 'Kiss Nail'){
                                    $assignee_list = $designer_list_kiss_nail;
                                }elseif ($design_group == 'Kiss Lash'){
                                    $assignee_list = $designer_list_kiss_lash;
                                }elseif ($design_group == 'Kiss Hair Care'){
                                    $assignee_list = $designer_list_kiss_hair;
                                }elseif ($design_group == 'Brand Design'){
                                    $assignee_list = $designer_list_others;
                                }elseif ($design_group == 'Industrial Design'){
                                    $assignee_list = $designer_list_industrial_design;
                                }elseif ($design_group == 'Production Design'){
                                    $assignee_list = $designer_list_production_design;
                                }

                                ?>
                                <div class="clearfix" id="{{$task->npd_design_request_type_id}}" style="border-left: 10px solid {{ $left_border_color }}; border-radius: 20px;">

                                    <div class="asset--grid-row">

                                        <div class="project-info">
                                            <ul class="project-info-list">
                                                <li><strong>{{ strtoupper(str_replace('_', ' ', $task->request_type)) }} #{{ $task->npd_design_request_type_id }}</strong> </li>
                                                <?php if ($left_border_color == 'white') $left_border_color = '#6c757d'; ?>
                                                <li><strong>Design Group: </strong> <b style="color: #b91d19;">{{$task->design_group}}</b></li>
                                        ...
                                        <ul class="project-members-list" style="padding-left: 170px;">
                                            ...
                                            <?php if($task->assignee_name != null){ ?>
                                            <li><strong>Assignee :</strong> <span class="assignee-bg">{{ $task->assignee_name }}</span></li>
                                            <?php } ?>
                                            <?php if($task->multiple_assignees != null){ ?>
                                            <li><strong>Multiple Assignees :</strong> <span class="multiple-assignees-bg">{{ $task->multiple_assignees }}</span></li>
                                            <?php } ?>
                                            ...
                                            <div class="project-action-icons">
                                                <ul class="project-icons">
                                                    <li>
                                                        ...
                                                        class="close"
                                                        data-id=""
                                                        data-request-type-id="{{ $task->npd_design_request_type_id }}"
                                                        data-request-type="{{ $task->request_type }}"
                                                        onclick="delete_npd_design_type($(this));">
                                    ...
                                    <div style="text-align: center;">
                                        <i id="arrow-{{$task->npd_design_request_type_id}}" class="dropdown fa-lg fa fa-angle-down" onclick="click_arrow(this, {{$task->npd_design_request_type_id}})"></i>
                                    </div>

                                    <div id="asset-id-{{$task->npd_design_request_type_id}}" class="box-body form_creator" data-asset-id="{{ $task->npd_design_request_type_id }}" style="display: none">
                                        <section>
                                            <div class="inner_box">
                                                <?php $data = [$task, $task->files, $design_group, $assignee_list]; ?>
                                                @include('admin.npd_design_request.npd_design_request', $data)
                                            </div>
                                        </section>
                                    </div>
                    ...
                    @if(!empty($project))
                        <?php if(Auth::user()->function == 'Product' || Auth::user()->function == 'Marketing' || Auth::user()->function == 'Admin') {?>

                        <div class="clearfix" id="asset_selector" style="display: block">
                        <div class="card box asset box-primary">
                            <div class="card-header" id="design_bg" style="height: 500px;">
                                <div class="row" style="display: flex; justify-content: center; padding: 5px 0 0 0;">
                                    <div id="add_task_btn" style="display: block">
                                        <a class="btn-lg btn-secondary add-row" style="color: #050000;
                                        background-color: #cdd3d8; border-radius: 1.25rem;"
                                           onclick="click_task_add_btn()">Add <b style="color: #b91d19;">NPD DESIGN REQUEST</b> Type</a>
                                    </div>
                                </div>

                                <div id="new_sub_request" style="display: none;">
                                    <div class="form-group">
                                        <span class="float-right">
                                            <a href="{{ url('admin/npd_design_request/'. $project->id .'/edit') }}">
                                                <i class="fa fa-times" style="font-size: 1.5rem;"></i>
                                            </a>
                                        </span>
                                    </div>
                                    @include('admin.npd_design_request.new.npd_design_request')
                                </div>
                            ...
                            <div class="col">
                                ...
                                <section id="add_note" class="notes" style="display: none;">
                                    <div class="write note">
                                        <form method="POST" action="{{ route('npd_design_request.add_note') }}" enctype="multipart/form-data">
                                            @csrf
    ...
    <script type="text/javascript">
        ...
        function delete_npd_design_type(el) {
            if (confirm("Are you sure to Delete? Are you sure you want to delete? Once deleted, this action cannot be undone and the data will no longer be accessible.") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/requestTypeRemove'); ?>"+"/"+id+"/"+type,
                    type: "GET",
                    datatype: "json",
        ...
        function remove_file(el) {
            if (confirm("Are you sure to Delete File?") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/fileRemove'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response == 'success'){
        ...
        function action_resubmit(el){
            ...
            if (confirm("Ensure all changes are saved before resubmitting.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/actionReSubmit'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_start(el){
            ...
            if (confirm("Your request status will change to 'In Progress'") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/actionInProgress'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_decline(el){

            if (confirm("Are you sure to Decline?") == true) {
                let id = $(el).attr('data-request-type-id');

                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/actionDecline'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function update_required(el){

            if (confirm("Are you sure to Revision?") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/updateRequired'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_review(el){
            ...
            if (confirm("The work has been updated for review, and the status will now change to 'Action Review'.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/actionReview'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_complete(el){
            ...
            if (confirm("The request is now complete, and no further changes can be made.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_design_request/actionComplete'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
