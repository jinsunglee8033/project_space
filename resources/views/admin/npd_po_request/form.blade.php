...
@section('content')
    ...
    <section class="section">
        ...
        <div class="section-header">
            <h1>NPD PO Request</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></div>
                <div class="breadcrumb-item"><a href="{{ url('admin/project') }}">NPD PO Request</a></div>
                <div class="breadcrumb-item active">NPD PO Request</div>
            </div>
        </div>

        <div class="section-body">
            <h2 class="section-title">NPD PO Request</h2>
            <div class="row">
                <div class="col-lg-7">
                    <div class="card">
                    ...
                    @if(!empty($npd_po_request_list))

                        <?php foreach ($npd_po_request_list as $task): ?>
{{--                        if Exist!!!   --}}
                        <?php
                        ...
                        ?>
                            ...
                            <div class="clearfix card" id="{{ $task->detail[0]->task_id }}" style="border-left: 10px solid {{$status_color}}; border-radius: 20px;">
                                ...
                                <div class="asset--grid-row">
                                    ...
                                    <div class="project-info">
                                        <ul class="project-info-list">
                                            <li><strong>NPD PO Request #{{$task->detail[0]->task_id}}</strong> </li>
                                            ...
                                            <?php if($task->detail[0]->due_date_urgent){ ?>
                                            <li><strong>Due Date (Urgent): </strong> {{ date('m/d/y', strtotime($task->detail[0]->due_date_urgent)) }}</li>
                                            <?php }else{ ?>
                                            <li><strong>Due Date: </strong> {{ date('m/d/y', strtotime($task->detail[0]->due_date)) }}</li>
                                            <?php } ?>
                                        </ul>
                                    </div>
                                    ...
                                    <ul class="project-members-list" style="padding-left: 170px;">
                                        ...
                                        <?php if($task->detail[0]->buyer){ ?>
                                        <li><strong>Buyer : </strong> <span class="asset-creator-bg" style="background-color: #dbe0ec;">{{ $task->buyer_name }}</span></li>
                                        <?php } ?>
                                        <?php if($task->detail[0]->due_date_revision){ ?>
                                        <li><strong>Due Date (Revision) : </strong>
                                            <span style="background-color: #eceadb;">{{ date('m/d/y', strtotime($task->detail[0]->due_date_revision)) }}</span>
                                            <span style="background-color: #eceadb; color: #b91d19;">{{ $task->detail[0]->revision_cnt }}</span>
                                        </li>
                                        <?php } ?>
                                    </ul>
                                            ...
                                            <ul class="project-icons">
                                                <li>
                                                    <a  href="javascript:void(0);"
                                                        ...
                                                        data-request-type="npd_po_request"
                                                        onclick="delete_npd_po_request_type($(this));">
                                ...
                                <div class="box-body form_creator">
                                    <section>
                                        <div class="inner_box">
                                             @include('admin.npd_po_request.npd_po_request', $data)
                                        </div>
                                    </section>
                                </div>
                            </div>
...
{{--                        if New!!!     --}}
                        <div class="clearfix card" id="" style="border-left: 10px solid #fbd102; border-radius: 20px;">
                            <div class="box-body form_creator">
                                <section>
                                    <div class="inner_box">
                                        @include('admin.npd_po_request.new.npd_po_request')
                                    </div>
                                </section>
                            </div>
                        </div>
                            ...
                            <div class="col">
                                ...
                                <section id="add_note" class="notes" style="display: none;">
                                    <div class="write note">
                                        <form method="POST" action="{{ route('npd_po_request.add_note') }}" enctype="multipart/form-data">
                                            @csrf
    ...
    <script type="text/javascript">
        ...
        function select_request_type(){
            ...
            if(request_type == 'mm_request'){
                $("#new_mm_request").fadeIn(600);
            }
        }
        ...
        function remove_file(el) {
            if (confirm("Are you sure to Delete File?") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_po_request/fileRemove'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
        ...
        function action_resubmit(el){
            if (confirm("Ensure all changes are saved before resubmitting.") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_po_request/actionReSubmit'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function final_price(el){
            if (confirm("Are you sure to Action Final Price?") == true) {
                let id = $(el).attr('data-task-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_po_request/actionFinalPrice'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_start(el){
            ...
            if (confirm("Your request status will change to 'In Progress'") == true) {
                let id = $(el).attr('data-task-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_po_request/actionInProgress'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_review(el){
            ...
            if (confirm("The work has been updated for review, and the status will now change to 'Action Review'.") == true) {
                let id = $(el).attr('data-task-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_po_request/actionReview'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_complete(el){
            ...
            if (confirm("The request is now complete, and no further changes can be made.") == true) {
                let id = $(el).attr('data-task-id');
                $.ajax({
                    url: "<?php echo url('/admin/npd_po_request/actionComplete'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function delete_npd_po_request_type(el) {
            if (confirm("Are you sure to Delete? Are you sure you want to delete? Once deleted, this action cannot be undone and the data will no longer be accessible.") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/npd_po_request/requestTypeRemove'); ?>"+"/"+id+"/"+type,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
