...
@section('content')
    ...
    <section class="section">
        ...
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item"><a href="{{ url('admin/project') }}">Legal Request</a></div>
                <div class="breadcrumb-item active">Legal Request</div>
            </div>
        </div>

        <div class="section-body">
            <h2 class="section-title">Legal Request</h2>
            <div class="row">
                <div class="col-lg-7">
                    <div class="card">
                    ...
                    @if(!empty($request_type_list))
                        ...
                        <?php foreach ($request_type_list as $task): ?>
                            ...
                            <div class="card assets_existing">
                                ...
                                ?>
                                <div class="clearfix" id="{{$task->legal_request_type_id}}" style="border-left: 10px solid {{ $left_border_color }}; border-radius: 20px;">

                                    <div class="asset--grid-row">

                                        <div class="project-info">
                                            <ul class="project-info-list">
                                                <li><strong>{{ strtoupper(str_replace('_', ' ', $task->request_type)) }} #{{ $task->legal_request_type_id }}</strong> </li>
                                            ...
                                            <div class="project-action-icons">
                                                <ul class="project-icons">
                                                    <li>
                                                        ...
                                                        class="close"
                                                        data-id=""
                                                        data-legal-request-type-id="{{ $task->legal_request_type_id }}"
                                                        data-legal-request-type="{{ $task->request_type }}"
                                                        onclick="delete_legal_request_type($(this));">
                                    ...
                                    <div style="text-align: center;">
                                        <i id="arrow-{{$task->legal_request_type_id}}" class="dropdown fa-lg fa fa-angle-down" onclick="click_arrow(this, {{$task->legal_request_type_id}})"></i>
                                    </div>

                                    <div id="asset-id-{{$task->legal_request_type_id}}" class="box-body form_creator" data-asset-id="{{ $task->legal_request_type_id }}" style="display: none">
                                        <section>
                                            <div class="inner_box">
                                                <?php $data = [$task, $task->files]; ?>
                                                @include('admin.legal_request.'.$task->request_type, $data)
                                            </div>
                                        </section>
                                    </div>
                    ...
                    @if(!empty($project))
                        ...
                        <div class="card box asset box-primary">
                            <div class="card-header" id="legal_bg" style="height: 500px;">
                                <div class="row" style="display: flex; justify-content: center; padding: 5px 0 0 0;">
                                    <div id="add_task_btn" style="display: block">
                                        <a class="btn-lg btn-secondary add-row" style="color: #050000; background-color: #cdd3d8; border-radius: 1.25rem;" onclick="click_task_add_btn()">Add <b style="color: #b91d19;">LEGAL REQUEST</b> Type</a>
                                    </div>
                                </div>
                                        ...
                                        <div class="inner_box">
                                            <div class="form-group">
                                                ...
                                                <span class="float-right">
                                                    <a href="{{ url('admin/legal_request/'. $project->id .'/edit') }}">
                                            ...
                                            <div id="new_trademark" style="display: none;">
                                                @include('admin.legal_request.new.trademark')
                                            </div>
                                            <div id="new_patent" style="display: none;">
                                                @include('admin.legal_request.new.patent')
                                            </div>
                                            <div id="new_claim" style="display: none;">
                                                @include('admin.legal_request.new.claim')
                                            </div>
                                            <div id="new_contract" style="display: none;">
                                                @include('admin.legal_request.new.contract')
                                            </div>
                                        </div>
                                    </section>
                                </div>
                            ...
                            <div class="col">
                                ...
                                <section id="add_note" class="notes" style="display: none;">
                                    <div class="write note">
                                        <form method="POST" action="{{ route('legal_request.add_note') }}" enctype="multipart/form-data">
                                            @csrf
    ...
    <script type="text/javascript">
        ...
        function select_request_type(){
            ...
            if(request_type == 'trademark'){
                $("#new_trademark").fadeIn(600);
                $("#new_patent").fadeOut();
                $("#new_claim").fadeOut();
                $("#new_contract").fadeOut();
            }else if(request_type == 'patent'){
                $("#new_trademark").fadeOut();
                $("#new_patent").fadeIn(600);
                $("#new_claim").fadeOut();
                $("#new_contract").fadeOut();
            }else if(request_type == 'claim'){
                $("#new_trademark").fadeOut();
                $("#new_patent").fadeOut();
                $("#new_claim").fadeIn(600);
                $("#new_contract").fadeOut();
            }else if(request_type == 'contract'){
                $("#new_trademark").fadeOut();
                $("#new_patent").fadeOut();
                $("#new_claim").fadeOut();
                $("#new_contract").fadeIn(600);
            }
            $("#legal_bg").height('');
        }
        ...
        function remove_file(el) {
            if (confirm("Are you sure to Delete File?") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/legal_request/fileRemove'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response == 'success'){
        ...
        function action_resubmit(el){
            ...
            if (confirm("Ensure all changes are saved before resubmitting.") == true) {
                ...
                $.ajax({
                    url: "<?php echo url('/admin/legal_request/actionReSubmit'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_start(el){
            ...
            if (confirm("Your request status will change to 'In Progress'") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/legal_request/actionInProgress'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_review(el){
            ...
            if (confirm("The work has been updated for review, and the status will now change to 'Action Review'.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/legal_request/actionReview'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
        ...
        function action_complete(el){
            ...
            if (confirm("The request is now complete, and no further changes can be made.") == true) {
                let id = $(el).attr('data-request-type-id');
                $.ajax({
                    url: "<?php echo url('/admin/legal_request/actionComplete'); ?>"+"/"+id,
                    type: "GET",
                    datatype: "json",
                    success: function(response) {
                        if(response != 'fail'){
