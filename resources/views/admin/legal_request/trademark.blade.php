...
<form method="POST" action="{{ route('legal_request.edit_trademark', $legal_request_type_id) }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="formulation_prescreen">
        ...
        <p style="color: #222; text-align: center; font-weight: bold;">TRADEMARK</p>

        <div class="form-group">
            <label>Description:</label>
{{--            <textarea class="form-control" style="height:100px;"--}}
{{--                      id="{{ $request_type }}_request_description"--}}
{{--                      name="{{ $request_type }}_description" required>{{ $data[0]->request_description }}</textarea>--}}
        ...
        <div class="form-group">
            ...
            <input type="text" name="description_of_goods" class="form-control" value="{{ $data[0]->description_of_goods }}" required>
        </div>

        <div class="form-group">
            <label>Trademark Owner: </label>
            <select class="form-control" name="trademark_owner">
                <option value="">Select</option>
                <?php foreach($trademark_owner_list as $val): ?>
                <option value="{{ $val }}" {{ $data[0]->trademark_owner == $val ? 'selected' : '' }} >
                    {{ $val }}
                </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="urgent_reason" name="urgent_reason" style="height: 100px;" required>{{ $data[0]->urgent_reason }}</textarea>
        </div>
        <?php } ?>
